...
<body>
  <div class="shell">
    ...
    <div id="accounts" class="tab-content active">
      <div class="sub-tabs">
        <button class="sub-tab active" data-subtab="acc-add">Tambah Akun</button>
        <button class="sub-tab" data-subtab="acc-search">Cari Akun</button>
        <button class="sub-tab" data-subtab="acc-edit">Edit Akun</button>
        <button class="sub-tab" data-subtab="acc-models">Status per Model</button>
      </div>

      <div id="acc-add" class="sub-tab-content active">
        <div class="panel">
          <h2>Tambah Akun Provider</h2>
          <form method="post" action="/admin/accounts/save">
            <input type="hidden" name="provider_account_id" value="{form_values['provider_account_id']}">
            <div class="form-grid">
              <label>Provider
                <input name="provider" value="{h(form_values['provider'])}" required placeholder="mis. nvidia, groq, z.ai">
              </label>
              <label>Account name
                <input name="account_name" value="{h(form_values['account_name'])}" required placeholder="mis. main-key-1">
              </label>
              <label>Model default
                <input name="model_name" value="{h(form_values['model_name'])}" required placeholder="mis. openai/gpt-oss-120b">
              </label>
              <label>Endpoint URL
                <input name="endpoint_url" value="{h(form_values['endpoint_url'])}" required placeholder="https://integrate.api.nvidia.com/v1">
              </label>
              <label>Usage method
                <input name="usage_method" value="{h(form_values['usage_method'])}" required placeholder="http_chat">
              </label>
              <label>API key
                <input name="api_key" type="password" value="" required>
              </label>
            </div>
      ...
      <div id="acc-search" class="sub-tab-content">
        <div class="panel">
          <h2>Cari Akun</h2>
        ...
        <div class="panel" style="margin-top:1rem;">
          <h2>Accounts ({summary['total_accounts']})</h2>
      ...
      <div id="acc-edit" class="sub-tab-content">
        {'<div class="panel"><h2>Edit Akun #' + str(form_values["provider_account_id"]) + ': ' + h(form_values["provider"]) + ' / ' + h(form_values["account_name"]) + '</h2>' if form_values["provider_account_id"] else '<div class="panel"><h2>Edit Akun</h2><div class="muted">Pilih akun dari tab <strong>Cari Akun</strong> lalu klik <strong>Edit</strong> untuk mengubah data akun.</div>'}
          {'<form method="post" action="/admin/accounts/save">' if form_values['provider_account_id'] else ''}
            <input type="hidden" name="provider_account_id" value="{form_values['provider_account_id']}">
            <div class="form-grid">
              <label>Provider
              ...
              <label>API key
                <input name="api_key" type="password" value="" placeholder="Kosongkan jika tidak ingin diganti">
              </label>
            </div>
            <div class="subtle">Kosongkan API key bila tidak ingin diganti.</div>
            <label>Extra headers JSON
              <textarea name="extra_headers_json">{h(form_values['extra_headers_json'])}</textarea>
            </label>
            <label>Notes
            ...
            {'<div class="button-row"><button type="submit" class="good">Simpan perubahan</button><button type="submit" formaction="/admin/accounts/test">Test koneksi</button><a class="link-button" href="/admin">Reset form</a></div>' if form_values['provider_account_id'] else ''}
          {'</form>' if form_values['provider_account_id'] else ''}
        </div>
      </div>

      <div id="acc-models" class="sub-tab-content">
        <div class="panel">
          <h2>Status per Model</h2>
          <div class="muted">Status runtime per model dari semua akun aktif.</div>
