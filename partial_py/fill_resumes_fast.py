...
def worker(q, api_key, key_id, thread_id):
...
"""
    ...
    while not q.empty():
        ...
        print(f"[Key-{key_id}-T{thread_id}] Processing {video_id}")
        
        if not t_path.exists():
            print(f"[Key-{key_id}-T{thread_id}] Missing: {t_path}")
            q.task_done()
            continue
        ...
        try:
            ...
            conn.commit()
            conn.close()
            print(f"[Key-{key_id}-T{thread_id}] Done: {video_id}")
        except Exception as e:
            print(f"[Key-{key_id}-T{thread_id}] Error {video_id}: {e}")
            time.sleep(1)
...
def main():
    ...
    print(f"Processing {len(tasks)} tasks using {len(keys)} keys and 30 threads total.")
    q = Queue()
    for t in tasks:
        q.put(t)
    ...
    for i, key in enumerate(keys):
        for t_idx in range(threads_per_key):
            t = threading.Thread(target=worker, args=(q, key, i+1, t_idx+1))
            t.start()
            threads.append(t)
