#!/usr/bin/env python3
"""
Sync Long Summary Script

Script ini menyalin summary yang lebih panjang/comprehensive dari resume files.

Source: /media/harry/128NEW1/GIT/yt_channel/out/{channel}/resume/{id}_{video_id}.md
Target: /media/harry/128NEW1/GIT/YOUTUBE/uploads/{channel}/resume/{video_id}.md

Database field: summary_file_path
"""
...
class LongSummarySync:
    """Synchronizer untuk long summary (resume.md files)"""
    ...
    def sync_summaries(self):
        """Sync semua resume files"""
        print("\n" + "=" * 60)
        print("📄 SYNC LONG SUMMARIES (RESUME.MD)")
        print("=" * 60)
        ...
        # Get all videos dengan resume
        cursor.execute("""
            SELECT 
                v.id, v.channel_id, v.video_id,
                v.link_resume
            FROM videos v
            WHERE v.link_resume IS NOT NULL 
              AND v.link_resume != ''
        """)
        
        videos = cursor.fetchall()
        print(f"📊 Found {len(videos)} videos with resume files")
        
        for video in videos:
            source_channel_id = video['channel_id']
            video_id = video['video_id']
            link_resume = video['link_resume']
            ...
            if not target_channel_id:
                self.stats['errors'].append(f"Video {video_id}: Channel {channel_slug} not in target")
                continue
            
            # Copy file
            self._copy_file(link_resume, channel_slug, video_id, target_channel_id)
    ...
    def _copy_file(self, source_rel_path: str, channel_slug: str,
        ...
        # Source: out/{channel}/resume/{id}_{video_id}.md
        source_file = self.source_base / channel_slug / source_rel_path
        
        # Target: uploads/{channel}/resume/{video_id}.md
        file_name = f"{video_id}.md"
        target_dir = self.target_base / channel_slug / "resume"
        ...
        # Update database with path relative to project root
        rel_path = str(Path(channel_slug) / "resume" / file_name)
    ...
    def print_report(self):
        """Print laporan"""
        print("\n" + "=" * 60)
        print("📊 LONG SUMMARY SYNC REPORT")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync long summary files"
    )
    parser.add_argument(
        "--source-db",
    ...
    print("=" * 60)
    print("📄 Long Summary Synchronizer")
    print("=" * 60)
    ...
    try:
        sync = LongSummarySync(args.source_db, args.target_db,
        ...
        print("\n" + "=" * 60)
        print("✅ Long summary sync completed!")
        print("=" * 60)
