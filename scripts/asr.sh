#!/bin/bash
# Wrapper script untuk ASR transcript YouTube via Groq/NVIDIA Whisper
...
echo "============================================="
echo "🎧 YouTube ASR Recovery Tool"
echo "============================================="
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --video-id ID         Process a single video"
            echo "  --channel-id ID       Process missing transcript videos from one channel"
            ...
            echo ""
            echo "Examples:"
            echo "  $0 --channel-id UC1234567890 --limit 20"
            echo "  $0 --video-id dQw4w9WgXcQ"
            echo "  $0 --providers groq,nvidia --language multi --chunk-seconds 45 --overlap-seconds 2"
            echo "  $0 --video-workers 2 --providers groq,nvidia"
            exit 0
            ;;
        *)
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ ASR transcript completed successfully"
else
    echo "❌ ASR transcript failed with exit code: $EXIT_CODE"
fi
echo "============================================="
exit $EXIT_CODE
